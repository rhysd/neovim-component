...
describe('neovim element', function () {
    ...
    it('can start without an error', function () {
        return this.app.client.getWindowCount().then((c: number) => assert.equal(c, 1))
            .then(() => this.app.browserWindow.isVisible()).then((b: boolean) => assert.isTrue(b))
            ...
            .then(() => this.app.client.getRenderProcessLogs()).then((logs: any[]) => {
                ...
                assert.equal(logs.filter(
                    m => !['net::ERR_FILE_NOT_FOUND', 'Electron Security Warning'].some(w => m.includes(w))).length,
                    0,
                    logs.toString(),
                );
            });
